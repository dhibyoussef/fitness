<?php
$pageTitle = $pageTitle ?? 'Delete Workout';
$workout = $workout ?? [];
...
<div class="container mx-auto py-8">
    ...
    <div class="card shadow-sm p-4 animate__animated animate__fadeInUp">
        <p>Are you sure you want to delete "<strong><?php echo htmlspecialchars($workout['name'] ?? ''); ?></strong>"? This action cannot be undone.</p>

        <form method="POST" action="/workout/delete/<?php echo htmlspecialchars($workout['id'] ?? ''); ?>">
            <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($csrf_token); ?>">
            <input type="hidden" name="confirm" value="yes">
            <button type="submit" class="btn btn-danger">Yes, Delete</button>
            <a href="/workouts/index" class="btn btn-secondary">No, Cancel</a>
        </form>
    </div>
